...
export default function Contact() {
  ...
  return (
    <section id="contact" className="pt-20 pb-28 px-6 text-center bg-slate-950 ">
      {/* Title + Subtitle */}
      <H2motion initial={{y:10,opacity:0}}
                  whileInView={{y:0,opacity:1}}
                  transition={{duration:0.8}}
                  viewport={{once:false,amount:0.3}} className="text-3xl font-bold mb-2 bg-gradient-to-r from-amber-200 via-amber-300 to-amber-600 bg-clip-text text-transparent relative
    ...
    after:rounded-full ">
      ...
      <Pmotion initial={{y:10,opacity:0}}
                  whileInView={{y:0,opacity:1}}
                  transition={{duration:0.8}}
                  viewport={{once:false,amount:0.3}} className="text-gray-400 max-w-2xl mx-auto mb-10">
